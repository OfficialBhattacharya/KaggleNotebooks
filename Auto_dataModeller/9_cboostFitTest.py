...
def check_gpu_availability():
    ...
    Enhanced to support multi-GPU detection and configuration for CatBoost.
    """
    gpu_info = {
    ...
    try:
        # Test if CatBoost can use GPU
        test_model = cb.CatBoostRegressor(task_type='GPU', iterations=1, verbose=False)
        # Try to fit a small dummy dataset
        X_dummy = np.random.randn(10, 2)
        y_dummy = np.random.randn(10)
        test_model.fit(X_dummy, y_dummy)
        gpu_info['catboost_gpu_available'] = True
        print(f"   CatBoost GPU Support: Available")
        logger.info("CatBoost GPU support available")
    except Exception as e:
        gpu_info['catboost_gpu_available'] = False
        print(f"   CatBoost GPU Support: Not available ({type(e).__name__})")
        logger.info(f"CatBoost GPU support not available: {type(e).__name__}")
...
def create_diagnostic_plots(fold_results, cv_predictions, y_train_original, X_test, test_predictions, 
    ...
    """
    Create comprehensive diagnostic plots for CatBoost model analysis.
    ...
    """
    logger.info("Creating diagnostic plots for CatBoost")
    
    fig, axes = plt.subplots(3, 3, figsize=(18, 15))
    fig.suptitle(f'CatBoost Model Diagnostic Analysis: {model_name} on {dataset_name}', 
    ...
    # 5. Feature Importance (CatBoost specific)
...
def fitPlotAndPredict(X_train, y_train, X_test, model, datasetName, xTransform, yTransform, modelName, cvScheme, time_column=None, X_train_full=None, y_train_full=None, cat_features=None):
    """
    Fit CatBoost model using cross-validation, generate diagnostic plots, and make predictions.
    ...
    X_test : array-like of shape (n_test_samples, n_features)
        Test feature matrix
    model : catboost estimator
        CatBoost model to fit
    datasetName : str
        Name of the dataset
    xTransform : str
    ...
    cat_features : list, optional
        List of categorical feature names or indices for CatBoost
    ...
    """
    print(f"\n{'='*80}")
    print(f"🚀 STARTING CATBOOST TRAINING: {modelName}")
    print(f"{'='*80}")
    ...
    if cat_features is not None:
        if hasattr(X_train, 'columns'):
            ...
            for cat_feat in cat_features:
                if isinstance(cat_feat, str) and cat_feat in X_train.columns:
                    cat_feature_indices.append(list(X_train.columns).index(cat_feat))
                elif isinstance(cat_feat, int):
                    cat_feature_indices.append(cat_feat)
        else:
            # Assume cat_features are already indices
            cat_feature_indices = [feat for feat in cat_features if isinstance(feat, int)]
        
        print(f"   🏷️  Categorical features: {len(cat_feature_indices)} features")
        logger.info(f"Categorical feature indices: {cat_feature_indices}")
    else:
        print(f"   🏷️  Categorical features: Auto-detection by CatBoost")
        logger.info("No categorical features specified - CatBoost will auto-detect")
    
    # Convert to numpy arrays if they are pandas objects (but keep original for CatBoost)
    ...
    if time_column is not None and time_data is not None:
        ...
        cv_splits = list(kf.split(X_train_np, duration_bins))
    else:
        logger.info("Using regular KFold cross-validation")
        kf = KFold(n_splits=n_folds, shuffle=True, random_state=42)
        cv_splits = list(kf.split(X_train_np))
    
    # Initialize storage for results
    fold_results = []
    oof_predictions = np.zeros(len(y_train_np))
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
        ...
        # Split data for this fold - use original data for CatBoost
        if hasattr(X_train_original, 'iloc'):
            X_fold_train = X_train_original.iloc[train_idx]
            X_fold_val = X_train_original.iloc[val_idx]
        else:
            X_fold_train = X_train_original[train_idx]
            X_fold_val = X_train_original[val_idx]
        
        if hasattr(y_train_original, 'iloc'):
            y_fold_train = y_train_original.iloc[train_idx]
            y_fold_val = y_train_original.iloc[val_idx]
        else:
            y_fold_train = y_train_original[train_idx]
            y_fold_val = y_train_original[val_idx]
        ...
        # Handle different model types, especially CatBoost with early stopping
        ...
        # Check for CatBoost GPU configuration
        gpu_detected = False
        if hasattr(fold_model, 'get_params'):
            ...
            if task_type == 'GPU':
                ...
                logger.info(f"✓ CatBoost using GPU acceleration (task_type='{task_type}')")
                gpu_detected = True
            else:
                print(f"   💻 Task Type: {task_type}")
        
        if not gpu_detected and gpu_info['catboost_gpu_available']:
        ...
        elif not gpu_info['catboost_gpu_available']:
            print(f"   ℹ️  No GPU available - using CPU")
            logger.info("ℹ️  No GPU available - using CPU")
        
        if 'CatBoost' in model_type:
            # CatBoost model with early stopping capability
            print(f"⚡ Training CatBoost...")
            logger.info("CatBoost model detected - providing validation set for early stopping")
                ...
                def after_iteration(self, info):
                    ...
                    if hasattr(info, 'metrics') and info.metrics:
                        ...
                        if 'validation' in info.metrics:
                            current_score = info.metrics['validation'][self.eval_metric][-1]
                        elif 'learn' in info.metrics:
                            current_score = info.metrics['learn'][self.eval_metric][-1]
                    ...
                    if current_time - self.last_print_time >= self.print_interval:
                        ...
                        if hasattr(info, 'metrics') and info.metrics:
                            if 'learn' in info.metrics and self.eval_metric in info.metrics['learn']:
                                train_score = f"{info.metrics['learn'][self.eval_metric][-1]:8.4f}"
                            if 'validation' in info.metrics and self.eval_metric in info.metrics['validation']:
                                val_score = f"{info.metrics['validation'][self.eval_metric][-1]:8.4f}"
                        
                        print(f"   📊 Iteration {iteration:4d} | Train {self.eval_metric}: {train_score} | Val {self.eval_metric}: {val_score} | GPU: {monitor_gpu_utilization()}")
            ...
            try:
                # Get eval metric from model
                eval_metric = 'RMSE'
                if hasattr(fold_model, 'get_params'):
                    params = fold_model.get_params()
                    eval_metric = params.get('eval_metric', 'RMSE')
                
                progress_callback = ProgressCallback(fold_idx, n_folds, eval_metric)
                ...
                fold_model.fit(X_fold_train, y_fold_train, cat_features=cat_feature_indices)
    ...
    cv_metrics = calculate_metrics(y_train_np, oof_predictions)
    ...
    # Handle CatBoost for final model
    if 'CatBoost' in model_type:
        print(f"⚡ Training final CatBoost model...")
        logger.info("Final CatBoost model training without early stopping for full training")
        ...
        final_model.fit(X_train_original, y_train_original, cat_features=cat_feature_indices, verbose=100)
    ...
    if use_full_dataset:
        X_final = X_train_full_original
        y_final = y_train_full_np
    ...
    final_test_predictions = final_model.predict(X_test_original)
    ...
    fig = create_diagnostic_plots(fold_results, oof_predictions, y_train_np, 
                                X_test_np, final_test_predictions, modelName, datasetName, feature_names)
    plt.show()
...
def create_gpu_optimized_catboost_model(task_type='regression', monotonic_constraints=None, interaction_constraints=None, cat_features=None, **kwargs):
    """
    Create GPU-optimized CatBoost models with proper configuration.
    ...
    **kwargs : dict
    ...
    --------
    model : catboost estimator
        Configured CatBoost model with GPU optimization if available
    """
    gpu_info = check_gpu_availability()
    ...
    try:
        ...
        # Enhanced CatBoost parameters optimized for performance
        default_params = {
        ...
        if monotonic_constraints is not None:
            if isinstance(monotonic_constraints, dict):
                # Convert dict to list format expected by CatBoost
        ...
        if interaction_constraints is not None:
            ...
            default_params['feature_weights'] = interaction_constraints  # CatBoost uses feature_weights for interactions
        ...
        if gpu_info['catboost_gpu_available']:
            ...
            default_params.update({
                ...
                'pinned_memory_size': 1024 * 1024 * 1024,  # 1GB pinned memory
                'gpu_cat_features_storage': 'GpuRam',
                'data_partition': 'FeatureParallel'
            })
            
            # Adjust parameters for GPU efficiency
            if gpu_info['gpu_count'] > 1:
                print(f"   🚀 Multi-GPU Setup: {gpu_info['gpu_count']} GPUs available")
                # CatBoost doesn't support multi-GPU directly, but we can optimize for single GPU
                default_params['devices'] = '0'  # Use first GPU
                print(f"   Configuration: Using GPU 0 (CatBoost single-GPU)")
            else:
                print(f"   Configuration: Single GPU training")
            
            print(f"   GPU RAM Part: {default_params['gpu_ram_part']}")
            print(f"   Pinned Memory: {default_params['pinned_memory_size'] / (1024**3):.1f} GB")
        ...
        if gpu_info['catboost_gpu_available']:
            logger.info(f"✓ Created CatBoost {task_type} model with GPU acceleration{constraint_info}")
            print(f"   ✅ GPU CatBoost {task_type} model created successfully{constraint_info}")
        else:
            logger.info(f"ℹ️  Created CatBoost {task_type} model for CPU (no GPU available){constraint_info}")
            print(f"   ✅ CPU CatBoost {task_type} model created successfully{constraint_info}")
