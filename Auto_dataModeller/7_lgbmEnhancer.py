...
def elbow_tune_n_estimators(X_train: pd.DataFrame, y_train: pd.Series, 
    ...
    for i, n_estimators in enumerate(n_estimators_range):
        ...
        try:
            if task == 'regression':
                model = LGBMRegressor(n_estimators=n_estimators, **base_params)
            else:
                model = LGBMClassifier(n_estimators=n_estimators, **base_params)
            
            # LightGBM specific training with early stopping
    ...
    plt.title(f'LightGBM Elbow Method for n_estimators Optimization\nTask: {task}')
    ...
    plot_path = f'elbow_n_estimators_lgbm_{task}.png'
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.show()
...
def optuna_objective(trial, X: pd.DataFrame, y: pd.Series, task: str, 
                    base_params: Dict, use_gpu: bool, cv_folds: int = 3) -> float:
    """
    ...
    """
    # Suggest LightGBM-specific hyperparameters
    params = {
    ...
    if use_gpu:
        ...
        params['max_bin'] = trial.suggest_int('max_bin', 63, 255)
    else:
        params['max_bin'] = trial.suggest_int('max_bin', 255, 512)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        try:
            if task == 'regression':
                ...
                model = LGBMClassifier(**params, n_estimators=500, random_state=42)
...
def run_optuna_optimization(X_train: pd.DataFrame, y_train: pd.Series, task: str,
                           base_params: Dict, use_gpu: bool = False, 
    ...
    """
    print(f"🔬 Starting LightGBM Optuna optimization with {n_trials} trials")
    print(f"   Task: {task}")
    ...
    objective = lambda trial: optuna_objective(
        trial, X_train, y_train, task, base_params, use_gpu, cv_folds
    )
...
def create_optimization_plots(study: optuna.Study, save_dir: str = "optimization_plots"):
    ...
    try:
        ...
        fig.write_image(save_path / "lgbm_optimization_history.png")
        
        # Parameter importance
        fig = optuna.visualization.plot_param_importances(study)
        fig.write_image(save_path / "lgbm_param_importances.png")
        
        # Parallel coordinate plot
        fig = optuna.visualization.plot_parallel_coordinate(study)
        fig.write_image(save_path / "lgbm_parallel_coordinate.png")
        
        print(f"📊 LightGBM optimization plots saved to {save_path}")
...
def train_final_model(X_train: pd.DataFrame, y_train: pd.Series, 
                     ...
                     task: str, best_params: Dict, 
                     model_name: str = "optimized_lgbm") -> Tuple[Any, Dict, Dict]:
    """
    Train the final LightGBM model with optimized hyperparameters.
    ...
    test_metrics : Dict
        Test metrics
    """
    print(f"🎯 Training final LightGBM model: {model_name}")
...
def create_feature_importance_plot(model, feature_names: List[str], 
    ...
    """Create feature importance visualization for LightGBM."""
    try:
        ...
        plt.title(f'Top {top_n} Feature Importances - {model_name} (LightGBM)')
        ...
        plt.tight_layout()
        plot_path = f"feature_importance_lgbm_{model_name}.png"
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.show()
        
        print(f"📊 LightGBM feature importance plot saved to {plot_path}")
...
def main():
    """
    Main function to run the enhanced LightGBM pipeline.
    """
    print("🚀 Enhanced LightGBM Pipeline Starting...")
    print("=" * 60)
    ...
    try:
        ...
        config['use_gpu'] = (gpu_info['cuda_available'] or gpu_info['opencl_available']) and config['use_gpu']
        ...
        if config['use_gpu']:
            base_params['device'] = gpu_info['recommended_device']
        ...
        if config['use_elbow_method']:
            ...
            optimal_estimators, elbow_model = elbow_tune_n_estimators(
                ...
                task, base_params, use_gpu=config['use_gpu']
            )
            
            base_params['n_estimators'] = optimal_estimators
            ...
            logger_instance.add_model(
                ...
                hyperparameters={'n_estimators': optimal_estimators},
                task_type=task,
        ...
        if config['use_optuna']:
            ...
            best_params, study = run_optuna_optimization(
                X_train, y_train, task, base_params,
        ...
        final_model, final_train_metrics, final_test_metrics = train_final_model(
            X_train, y_train, X_test, y_test, task, best_params, "final_optimized_lgbm"
        )
        ...
        print("\n✅ Enhanced LightGBM pipeline completed successfully!")
...
if __name__ == "__main__":
    # Example usage
    print("🌟 LightGBM Enhanced Pipeline")
    print("=" * 60)
    print("This script provides comprehensive LightGBM model optimization including:")
    print("• GPU acceleration support (CUDA/OpenCL)")
    print("• Elbow method for n_estimators optimization")
    print("• Optuna hyperparameter optimization")
    ...
    try:
        ...
        # Display key LightGBM-specific optimizations
        if 'device' in results['best_params']:
            print(f"\n🎮 GPU Configuration: {results['best_params']['device']}")
        if 'num_leaves' in results['best_params']:
            print(f"🌿 Optimized num_leaves: {results['best_params']['num_leaves']}")
        if 'feature_fraction' in results['best_params']:
            print(f"📊 Optimized feature_fraction: {results['best_params']['feature_fraction']:.3f}")
        ...
        print("• pyopencl (for OpenCL support)")
...
python 7_lgbmEnhancer.py

# Or import and use programmatically:
from lgbmEnhancer import main, check_gpu_availability, load_data

# Check GPU availability
gpu_info = check_gpu_availability()
print(f"GPU available: {gpu_info['cuda_available'] or gpu_info['opencl_available']}")
...
Key Features:
-------------
1. **GPU Acceleration**: Automatic detection and configuration for CUDA/OpenCL
2. **Elbow Method**: Optimal n_estimators selection using validation curves
3. **Optuna Optimization**: Advanced hyperparameter tuning with pruning
4. **LightGBM-Specific Parameters**: 
   - num_leaves, feature_fraction, bagging_fraction
   - path_smooth, max_bin optimization
   - GPU-specific memory optimizations
5. **Comprehensive Evaluation**: Multiple metrics for regression/classification
6. **Model Logging**: Track and compare different optimization strategies
7. **Visualization**: Feature importance, optimization history, model comparison
8. **Error Handling**: Robust error handling with fallback options
...
• path_smooth: Path smoothing parameter
• max_bin: Maximum number of bins for feature values

GPU Optimizations:
------------------
• Automatic device detection (CUDA/OpenCL)
• Memory-optimized parameters for GPU training
• Reduced max_bin for GPU memory efficiency
